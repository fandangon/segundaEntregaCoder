...
document.addEventListener("DOMContentLoaded", () => {
  mostrarProductosBebe();
});
document.addEventListener("DOMContentLoaded", () => {
  mostrarProductosCuidado();
});
document.addEventListener("DOMContentLoaded", () => {
  mostrarProductosDermo();
});

document.addEventListener("DOMContentLoaded", () => {
  mostrarProductosPerfumes();
});

document.addEventListener("DOMContentLoaded", () => {
  mostrarProductosSuplementos();
});
const cuidado = document.querySelector(".arregloProductosCuidado");
const suplemento = document.querySelector(".arregloProductosSuplemento");
const bebe = document.querySelector(".arregloProductosBebe");
const dermo = document.querySelector(".arregloProductosDermo");
const perfumes = document.querySelector(".arregloProductosPerfume");
const botonCarro = document.querySelector(".btnCarro");
const botonBorrarCarro = document.querySelector(".btnBorrarCarro");

function mostrarProductosBebe() {
  for (const items of productosBebe) {
    ...
    btnAgregarCarrito.onclick = () => {
      agregarCarrito(items.idHtml, items.id);
      swal({
...
function mostrarProductosCuidado() {
  for (const items of productosCuidado) {
    const divProductos = document.createElement("div");
    divProductos.classList.add("card");
    const imgProducto = document.createElement("img");
    imgProducto.classList.add("imgProducto");
...
function mostrarProductosDermo() {
  for (const items of productosDermo) {
    const divProductos = document.createElement("div");
    divProductos.classList.add("card");
    const imgProducto = document.createElement("img");
    imgProducto.classList.add("imgProducto");
...
function mostrarProductosPerfumes() {
  for (const items of productosPerfumes) {
    const divProductos = document.createElement("div");
    divProductos.classList.add("card");
    const imgProducto = document.createElement("img");
    imgProducto.classList.add("imgProducto");
...
function mostrarProductosSuplementos() {
  for (const items of productosSuplementos) {
    const divProductos = document.createElement("div");
    divProductos.classList.add("card");
    const imgProducto = document.createElement("img");
    imgProducto.classList.add("imgProducto");
    ...
    divProductos.appendChild(precio);
    divProductos.appendChild(btnAgregarCarrito);
    suplemento.appendChild(divProductos);
  }
}
...
function agregarCarrito(id, id1) {
  switch (id) {
    case 1:
      productoEncontrado = productosCuidado.find(
        (productos) => productos.id === id1
      );
      ...
      break;
    case 2:
      productoEncontrado = productosSuplementos.find(
        (productos) => productos.id === id1
      );
      productosCarritos.push(productoEncontrado);
      guardarLocalmente("productos", JSON.stringify(productosCarritos));
      ...
      break;
    case 3:
      productoEncontrado = productosBebe.find(
        (productos) => productos.id === id1
      );
      productosCarritos.push(productoEncontrado);
      guardarLocalmente("productos", JSON.stringify(productosCarritos));
      ...
      break;
    case 4:
      productoEncontrado = productosDermo.find(
        (productos) => productos.id === id1
      );
      productosCarritos.push(productoEncontrado);
      guardarLocalmente("productos", JSON.stringify(productosCarritos));
      ...
      break;
    case 5:
      productoEncontrado = productosPerfumes.find(
        (productos) => productos.id === id1
      );
      productosCarritos.push(productoEncontrado);
      guardarLocalmente("productos", JSON.stringify(productosCarritos));
...
recuperarLs = () => {
  ...
  if (recuperar) {
    recuperar.forEach((element) => agregarCarrito(element.id, element.idHtml));
  }
};
recuperarLs();
...
btnCarro.onclick = () => {
  swal({
    ...
    contado
      ? (suma = suma * 1.21)(
          swal(
            `La suma de los productos del carrito pagando en Crédito es $${suma.toFixed(
              2
            )}`,
            { icon: "success" }
          )
      ...
      : swal(
          `La suma de los productos del carrito pagando en Efectivo / Debito es $${suma.toFixed(
            2
          )}`,
          { icon: "success" }
        );
